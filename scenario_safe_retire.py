...
class ScenarioSafeRetire(Scenario):
    ...
    def __init__(self, p):
        ...
        self.early_shift_years = (
            10  # Start shifting to bonds 10 years before retirement
        )
        self.l3_proportion = 0.4  # Same as L3Broker which performed well
        self.initial_bond_allocation = 0.3  # Higher initial bond allocation
        self.floor_withdrawal_pct = 0.03  # Lower minimum withdrawal rate (3%)
        self.ceiling_withdrawal_pct = 0.045  # Lower maximum withdrawal rate (4.5%)
        self.target_withdrawal_pct = 0.035  # Lower target withdrawal rate (3.5%)
        self.market_sensitivity = (
            0.7  # Higher market sensitivity (more responsive to market conditions)
        )
    ...
    def decumulate_year(
        ...
        # Dynamic withdrawal strategy with safety buffer
        total_portfolio = pot.br_eq + pot.br_bd + pot.l3_eq
        available_portfolio = total_portfolio * (1 - self.safety_buffer)

        # Base withdrawal on available portfolio value and target rate
        base_withdrawal = available_portfolio * self.target_withdrawal_pct
        ...
        floor_withdrawal = available_portfolio * self.floor_withdrawal_pct
        ceiling_withdrawal = available_portfolio * self.ceiling_withdrawal_pct
